...
class LGChatMessage : NSObject {
    ...
    */
    ...
    required init (content: String, sentBy: SentBy, timeStamp: NSTimeInterval? = nil, userName: String, date: String){
        self.sentBy = sentBy
    ...
    convenience init (content: String, sentByString: String, userName: String, date: String) {
        if let sentBy = SentBy(rawValue: sentByString) {
            self.init(content: content, sentBy: sentBy, timeStamp: nil, userName: userName, date: date)
        } else {
            fatalError("LGChatMessage.FatalError : Initialization : Incompatible string set to SentByString!")
        }
    }
    
    convenience init (content: String, sentByString: String, timeStamp: NSTimeInterval, userName: String, date: String) {
        if let sentBy = SentBy(rawValue: sentByString) {
            self.init(content: content, sentBy: sentBy, timeStamp: timeStamp, userName: userName, date: date)
        } else {
            fatalError("LGChatMessage.FatalError : Initialization : Incompatible string set to SentByString!")
        }
    }
...
class LGChatMessageCell : UITableViewCell {
    ...
    struct Appearance {
        static var opponentColor = UIColor(red: 0.142954, green: 0.60323, blue: 0.862548, alpha: 0)
        static var userColor = UIColor(red: 0.14726, green: 0.838161, blue: 0.533935, alpha: 0)
        static var font: UIFont = UIFont.systemFontOfSize(15.0)
    }
    ...
    */
    ...
    private lazy var userTextView: UITextView = {
        ...
        userTextView.bounds.size = CGSize(width: 200, height: self.minimumHeight)
        let halfWidth = CGRectGetWidth(userTextView.bounds) / 2.0
        let halfHeight = CGRectGetHeight(userTextView.bounds) / 2.0
        
        // Center the imageview vertically to the textView when it is singleLine
        let textViewSingleLineCenter = self.textView.textContainerInset.top + (Appearance.font.lineHeight / 2.0)
        userTextView.center = CGPointMake(self.padding + halfWidth, textViewSingleLineCenter)
        userTextView.backgroundColor = UIColor.clearColor();
        userTextView.alpha = 0.6;
        userTextView.layer.masksToBounds = true
        self.contentView.addSubview(userTextView)
        return userTextView
    }()
    ...
    */
    func setupWithMessage(message: LGChatMessage) -> CGSize {
        ...
        self.userTextView.text = message.userName + "  " + message.date
        return size
    }
    ...
    private func styleTextViewForSentBy(sentBy: LGChatMessage.SentBy) {
        ...
        case .Opponent:
            ...
            self.userTextView.hidden = false
            CGRectGetWidth(self.userTextView.bounds)
            
        case .User:
            self.opponentImageView.hidden = true
            self.userTextView.hidden = true
...
class LGChatController : UIViewController, UITableViewDelegate, UITableViewDataSource, LGChatInputDelegate {
    ...
    */
    ...
    func chatInput(chatInput: LGChatInput, didSendMessage message: String) {
        let newMessage = LGChatMessage(content: message, sentBy: .User, userName: "", date: "")
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        cell.backgroundColor = message.sentBy == .Opponent ? UIColor.yellowColor() : UIColor.redColor()
        return cell;
    }
