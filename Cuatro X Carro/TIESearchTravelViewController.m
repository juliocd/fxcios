...
- (NSArray *)results
{
    if (!results)
    {
        ...
        for (NSInteger i=0; i<4; i++)
            [arr addObject:[NSString stringWithFormat:@"Ruta %ld", (long)i]];
        results = arr;
    }
    return results;
}
...
- (void)viewDidLoad {
    self.navigationController.navigationBar.topItem.title = @"Buscar";
    ...
    marker.map = self.searchRouteMap;
}
